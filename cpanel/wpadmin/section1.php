...
if (isset($_GET['action']) && $_GET['action'] === 'submit') {
    ...
    $g_desc = $db->CleanDBData($_POST['g_desc']);
    $g_detail = $db->CleanDBData($_POST['g_detail']);
    $g_video = $db->CleanDBData($_POST['g_video']);
    ...
    if ($_FILES["g_background"]["tmp_name"]) {
        ...
        $sqlu2 = "update section1 set g_background=?,g_title=?,g_desc=?,g_detail=?,g_video=? where sid=?";
        $values2 = array($basename, $g_title, $g_desc,$g_detail, $g_video, $sid);

    } else {
        $sqlu2 = "update section1 set g_background=?,g_title=?,g_desc=?,g_detail=?,g_video=? where sid=?";
        $values2 = array($g_background_old, $g_title, $g_desc,$g_detail, $g_video, $sid);
    }
    ...
    $RecDataSQL = "SELECT * FROM section1 WHERE `sid` = ? LIMIT 1";
  ...
  <section id="container">
    ...
    <section id="main-content">
      <section class="wrapper site-min-height">
        <h3><i class="fa fa-angle-right"></i> Content Section 1</h3>
        <div class="row mt">
          <div class="col-lg-12">
          <div class="form-panel">
              <form class="form-horizontal style-form" method="post" action="section1.php?action=submit" enctype="multipart/form-data">
...
?>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <input type="text" class="form-control" name="g_desc" value="<?php echo $RecData[0]['g_desc']; ?>">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-2 control-label">Detail</label>
                  <div class="col-sm-9">
                    <input type="text" class="form-control" name="g_detail" value="<?php echo $RecData[0]['g_detail']; ?>">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-2 control-label">Video</label>
                  <div class="col-sm-9">
                    <input type="text" class="form-control" name="g_video" value="<?php echo $RecData[0]['g_video']; ?>">
                  </div>
                </div>
                <div class="form-group">
